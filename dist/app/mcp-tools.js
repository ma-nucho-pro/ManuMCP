...
async function guardedContent(services, extra, operation) {
    try {
...
function assertControlProfile(services) {
...
async function controlCwd(services, rawPath) {
...
async function existingControlTarget(services, rawPath) {
...
function controlAction(services, extra, operation, digestInput, summary, token, confirmed, action) {
    if ((token === undefined && confirmed) || (token !== undefined && !confirmed)) {
        return guarded(services, extra, async () => {
            throw new TunnelGPTError("CONFIRMATION_INVALID", "Usa confirmed=true junto con el confirmationToken, o no envíes ninguno para solicitar una vista previa.");
        });
