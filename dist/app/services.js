...
export async function createServices(config) {
    ...
    return {
        ...
        inFlightConfirmationTokens: new Set(),
    };
}
export async function runBounded(services, signal, operation) {
...
export async function runConfirmed(services, token, operation) {
    if (token === undefined)
        return operation();
    const now = Date.now();
