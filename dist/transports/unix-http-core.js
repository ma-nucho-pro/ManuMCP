...
class HttpEnvelopeError extends Error {
    status;
    rpcCode;
    constructor(status, rpcCode, message) {
        super(message);
...
function sendRpcError(res, status, rpcCode, message) {
    sendJson(res, status, {
        jsonrpc: "2.0",
...
function sameSocketIdentity(stat, identity) {
...
async function probeUnixSocket(socketPath) {
...
async function prepareSocketPath(socketPath) {
    ...
    const identity = { dev: first.dev, ino: first.ino };
...
function requestAcceptsMcp(req) {
...
export class UnixHttpMcpServer {
    ...
    #handler;
    #admission;
    #instancesByEra = { legacy: 0, modern: 0 };
    #ready = false;
    #started = false;
    #closed = false;
    #socketIdentity;
    ...
    async #handle(req, res) {
        ...
        const leaveQueue = () => queueAbort.abort();
