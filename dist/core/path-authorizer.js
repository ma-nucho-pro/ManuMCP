...
export class PathAuthorizer {
    ...
    #rootsByAlias;
    ignorePolicy;
    constructor(config, roots, ignorePolicy) {
        this.#config = config;
        this.#roots = roots;
    ...
    static async create(config) {
        ...
        const dynamicDenied = [config.configFilePath];
    ...
    async authorizeExisting(input, options = {}) {
    ...
    async authorizeNew(input, options = {}) {
        const parsed = this.parsePath(input, options.rootAlias);
    ...
    async inspectEntry(input, rootAlias) {
    ...
    async #assertPolicies(root, absolutePath, relativePath, isDirectory, applyIgnore) {
    ...
    async #assertNoSymlinkComponents(root, target, includeTarget) {
    ...
    #authorized(root, absolutePath, relativePath) {
