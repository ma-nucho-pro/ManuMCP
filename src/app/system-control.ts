...
function unsupportedOnThisPlatform(capability: string): never {
    throw new TunnelGPTError("INVALID_ARGUMENT", `${capability} no está disponible en ${process.platform}.`);
}

function outputError(file: string, error: unknown): TunnelGPTError {
    const message = error instanceof Error ? error.message : "Error desconocido.";
    return new TunnelGPTError("INTERNAL_ERROR", `No se pudo ejecutar ${file}.`, { reason: message.slice(0, 512) });
}

function parseJsonOutput<T>(file: string, output: string, fallback: T): T {
    try {
        return JSON.parse(output.trim() || JSON.stringify(fallback)) as T;
    }
    catch (error) {
...
function macWindowParts(handle: number): { pid: number; index: number } {
...
const MAC_LETTER_CODES: Readonly<Record<string, number>> = {
...
const WINDOWS_KEY_CODES: Readonly<Record<string, number>> = {
...
const MAC_KEY_CODES: Readonly<Record<string, number>> = {
...
export class SystemControl {
    ...
    public constructor(config: ManuMcpConfig) {
        this.#config = config;
    ...
    public async executeCommand(args: {
        ...
        timeout.unref();
        try {
            const close = await new Promise<{ exitCode: number | null; signal: NodeJS.Signals | null }>((resolve, reject) => {
    ...
    public async launchApplication(args: {
        executable: string;
        arguments: readonly string[];
        cwd: string;
    }): Promise<{ ok: true; pid: number | null; executable: string; arguments: readonly string[]; cwd: string }> {
    ...
    public async terminateProcess(pid: number, force: boolean): Promise<{ ok: true; pid: number; force: boolean; output: string }> {
    ...
    public async listStorageVolumes(): Promise<{ ok: true; volumes: readonly VolumeInfo[] }> {
        if (process.platform === "win32") {
            const volumes = this.#config.access.allowedRoots
                ...
                .map((root) => ({ alias: root.alias, name: root.alias === this.#config.pcAlias ? `Unidad ${root.path.slice(0, 2)}` : `Unidad ${root.alias.slice(this.#config.pcAlias.length + 1).toUpperCase()}:`, path: root.path, accessPath: `${root.alias}:/`, mounted: true as const }));
            return { ok: true, volumes };
        }
        const volumes: VolumeInfo[] = [{ alias: this.#config.pcAlias, name: "Sistema de archivos del equipo", path: this.#config.pcPath, accessPath: `${this.#config.pcAlias}:/`, mounted: true }];
    ...
    public async listWindows(): Promise<{ ok: true; windows: readonly WindowInfo[] }> {
        if (process.platform === "win32")
        ...
        if (process.platform === "darwin") {
            ...
            return { ok: true, windows: parseJsonOutput<WindowInfo[]>(MAC_OSASCRIPT, output.stdout, []) };
        }
        unsupportedOnThisPlatform("La enumeración de ventanas");
    }

    public async focusWindow(handle: number): Promise<{ ok: true; handle: number }> {
        if (process.platform === "win32") {
...
'focused'`;
    ...
    public async closeWindow(handle: number): Promise<{ ok: true; handle: number }> {
        if (process.platform === "win32") {
            const script = String.raw`Add-Type @'
using System;
using System.Runtime.InteropServices;
...
'close-requested'`;
    ...
    public async getScreenInfo(): Promise<{ ok: true; screens: readonly ScreenInfo[] }> {
        if (process.platform === "win32") {
...
$items | ConvertTo-Json -Compress`;
            ...
            const parsed = parseJsonOutput<ScreenInfo | ScreenInfo[]>(this.#powershellPath, output.stdout, [] as ScreenInfo[]);
        ...
        if (process.platform === "darwin") {
            ...
            const parsed = parseJsonOutput<unknown>("/usr/sbin/system_profiler", raw.stdout, {});
    ...
    public async captureScreen(screenIndex: number | undefined, allScreens: boolean): Promise<ScreenCapture> {
        if (process.platform === "win32")
    ...
    public async getCursorPosition(): Promise<{ ok: true; x: number; y: number }> {
        if (process.platform === "win32") {
            const script = String.raw`Add-Type -AssemblyName System.Windows.Forms
...
[pscustomobject]@{ x = [int]$position.X; y = [int]$position.Y } | ConvertTo-Json -Compress`;
            ...
            return parseJsonOutput<{ ok: true; x: number; y: number }>(this.#powershellPath, output.stdout, { ok: true, x: 0, y: 0 });
        }
        if (process.platform === "darwin") {
            ...
            return parseJsonOutput<{ ok: true; x: number; y: number }>(MAC_OSASCRIPT, output.stdout, { ok: true, x: 0, y: 0 });
        }
        unsupportedOnThisPlatform("La posición del cursor");
    }

    public async controlMouse(args: {
        ...
        clicks?: number;
        delta?: number;
    }): Promise<{ ok: true; action: string; position: { x: number; y: number } }> {
...
[pscustomobject]@{ x = [int]$position.X; y = [int]$position.Y } | ConvertTo-Json -Compress`;
            const output = await this.runPowerShell(script, 64 * 1024);
            return { ok: true, action: args.action, position: parseJsonOutput<{ x: number; y: number }>(this.#powershellPath, output.stdout, { x, y }) };
        }
        if (process.platform === "darwin") {
            ...
            return { ok: true, action: args.action, position: parseJsonOutput<{ x: number; y: number }>(MAC_OSASCRIPT, output.stdout, { x, y }) };
        }
        unsupportedOnThisPlatform("El control del ratón");
    }

    public async typeText(text: string): Promise<{ ok: true; characters: number }> {
        if (process.platform === "win32") {
...
[ManuMcpKeyboard]::Unicode($text)`;
    ...
    public async hotkey(keys: readonly number[]): Promise<{ ok: true; keys: readonly number[] }> {
...
[ManuMcpHotkey]::Combo([ushort[]]@(${keyValues}))`;
    ...
    private resolveCommand(shell: CommandShell, command: string): ResolvedCommand {
        if (process.platform === "win32") {
            if (shell === "powershell") {
    ...
    private async spawnDetached<T extends { executable: string; arguments: readonly string[]; cwd: string }>(file: string, argumentsValue: readonly string[], cwd: string, original: T): Promise<{ ok: true; pid: number | null; executable: string; arguments: readonly string[]; cwd: string }> {
    ...
    private async listWindowsWindows(): Promise<{ ok: true; windows: readonly WindowInfo[] }> {
        const script = String.raw`Add-Type @'
using System;
using System.Collections.Generic;
using System.Runtime.InteropServices;
...
if ($null -eq $rows) { '[]' } else { @($rows) | ConvertTo-Json -Compress }`;
        ...
        const parsed = parseJsonOutput<WindowInfo | WindowInfo[]>(this.#powershellPath, output.stdout, [] as WindowInfo[]);
        return { ok: true, windows: Array.isArray(parsed) ? parsed : [parsed] };
    }

    private async captureScreenWindows(screenIndex: number | undefined, allScreens: boolean): Promise<ScreenCapture> {
...
try { $graphics.CopyFromScreen([int]$bounds.Left, [int]$bounds.Top, 0, 0, $bitmap.Size); $bitmap.Save($stream, [System.Drawing.Imaging.ImageFormat]::Png); [pscustomobject]@{ width = [int]$bounds.Width; height = [int]$bounds.Height; screen = $label; data = [Convert]::ToBase64String($stream.ToArray()) } | ConvertTo-Json -Compress }
        ...
        const parsed = parseJsonOutput<{ data?: unknown; width?: unknown; height?: unknown; screen?: unknown }>(this.#powershellPath, output.stdout, {});
    ...
    private async runJxa(script: string, maxBuffer: number): Promise<ExecutableResult> {
        if (process.platform !== "darwin")
            unsupportedOnThisPlatform("AppleScript/JXA");
        return this.runExecutable(MAC_OSASCRIPT, ["-l", "JavaScript", "-e", script], true, maxBuffer);
    }

    private async runPowerShell(script: string, maxBuffer: number): Promise<ExecutableResult> {
        if (process.platform !== "win32")
    ...
    private async runExecutable(file: string, argumentsValue: readonly string[], throwOnError = true, maxBuffer = 256 * 1024): Promise<ExecutableResult> {
        return new Promise((resolve, reject) => {
            execFile(file, [...argumentsValue], { windowsHide: process.platform === "win32", maxBuffer, encoding: "utf8" }, (error, stdout, stderr) => {
                ...
                const result: ExecutableResult = { stdout: String(stdout), stderr: String(stderr), exitCode };
...
export function keyCodeFromName(value: string): number {
