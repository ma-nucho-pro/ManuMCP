...
function unsupportedOnThisPlatform(): never {
...
if ($null -eq $rows) { '[]' } else { @($rows) | ConvertTo-Json -Compress }`;
        ...
        let parsed: unknown;
        try {
            parsed = JSON.parse(output.stdout.trim() || "[]") as unknown;
        }
        catch (error) {
            throw outputError(this.#powershellPath, error);
        }
        const windows = (Array.isArray(parsed) ? parsed : [parsed]) as WindowInfo[];
...
$items | ConvertTo-Json -Compress`;
        ...
        const screens = (Array.isArray(parsed) ? parsed : [parsed]) as ScreenInfo[];
...
try {
  ...
  $bitmap.Dispose()
        ...
        const value = parsed as { data?: unknown; width?: unknown; height?: unknown; screen?: unknown };
...
[pscustomobject]@{ x = [int]$position.X; y = [int]$position.Y } | ConvertTo-Json -Compress`;
        ...
        return JSON.parse(output.stdout.trim()) as { ok: true; x: number; y: number };
    }
...
[pscustomobject]@{ x = [int]$position.X; y = [int]$position.Y } | ConvertTo-Json -Compress`;
        const output = await this.runPowerShell(script, 64 * 1024);
        return { ok: true, action: args.action, position: JSON.parse(output.stdout.trim()) as { x: number; y: number } };
    }
...
[ManuMcpHotkey]::Combo([ushort[]]@(${keyValues}))`;
    ...
    async runPowerShell(script: string, maxBuffer: number): Promise<ExecutableResult> {
    ...
    async runExecutable(file: string, args: readonly string[], throwOnError = true, maxBuffer = 256 * 1024): Promise<ExecutableResult> {
        return new Promise((resolve, reject) => {
