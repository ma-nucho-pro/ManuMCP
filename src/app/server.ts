...
import { loadConfig, type ManuMcpConfig } from "./config.js";
import { registerManuMcpTools } from "./mcp-tools.js";
import { createServices, type ManuMcpServices } from "./services.js";

const MAX_HTTP_BODY_BYTES = 2 * 1024 * 1024;
const startedAt = new Date().toISOString();

function createMcpServer(services: ManuMcpServices): McpServer {
...
function healthPayload(config: ManuMcpConfig): Record<string, unknown> {
    return {
        ok: true,
        name: config.name,
...
async function handleMcpRequest(request: IncomingMessage, response: ServerResponse, services: ManuMcpServices): Promise<void> {
...
async function startHttp(config: ManuMcpConfig, services: ManuMcpServices): Promise<void> {
...
async function startStdio(config: ManuMcpConfig, services: ManuMcpServices): Promise<void> {
    const mcpServer = createMcpServer(services);
