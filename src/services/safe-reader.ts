...
export class SafeReader {
    public async readText(authorized: AuthorizedPath, request: ReadTextRequest): Promise<ReadTextResult> {
        ...
        try { handle = await fs.open(authorized.absolutePath, fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW ?? 0)); }
        catch (error) {
            throw new TunnelGPTError((error as NodeJS.ErrnoException).code === "ELOOP" ? "SYMLINK_DENIED" : "NOT_FOUND", "File cannot be opened safely.");
        }
        try {
            const stat = await handle.stat({ bigint: true });
            if (!stat.isFile()) throw new TunnelGPTError("SPECIAL_FILE_DENIED", "Only regular files can be read.");
            ...
            let next: { line: number; column: number } | undefined;
            ...
            const append = (value: string) => {
                if (!inRange() || next !== undefined) return;
                ...
                if (outputBytes + bytes > request.maxBytes) { next = { line, column }; return; }
                if (fresh) { entry = { line, text: "" }; selected.push(entry); }
                entry!.text += value;
                outputBytes += bytes;
            };
            const consume = (text: string) => {
                ...
                for (const char of text) {
                    ...
                    if (char === "\r") { pendingCr = true; continue; }
                    if (char === "\n") {
                        // Empty lines have a visible numbered entry, too.
                        if (column === 1) append("");
                        totalLines = line;
                        line++; column = 1;
                    } else { append(char); column++; totalLines = line; }
                }
            };
            let lastCharacter = "";
            for (;;) {
                ...
                position += bytesRead; hash.update(bytes); checkBinary(bytes);
                let decoded: string;
                try { decoded = decoder.decode(bytes, { stream: true }); }
                catch { throw new TunnelGPTError("UNSUPPORTED_ENCODING", "File is not valid UTF-8."); }
            ...
            try { consume(decoder.decode()); }
                ...
                if (error instanceof TunnelGPTError) throw error;
                throw new TunnelGPTError("UNSUPPORTED_ENCODING", "File is not valid UTF-8.");
            }
            if (pendingCr) { append("\r"); totalLines = line; }
            if (!position) { append(""); totalLines = 1; }
            ...
            if (containsSecret) throw new TunnelGPTError("SECRET_CONTENT_BLOCKED", "Potential secret content was blocked.");
            if (next && !selected.length) throw new TunnelGPTError("LIMIT_EXCEEDED", "Increase maxBytes to fit a line label and one Unicode character.", { minimumBytes: countUtf8Bytes(`${next.line}: `) + 4 });
