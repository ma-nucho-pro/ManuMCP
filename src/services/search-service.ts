...
export class SearchService {
    ...
    public constructor(walker: Walker, authorizer: PathAuthorizer, reader: SafeReader) {
        this.#walker = walker;
        this.#authorizer = authorizer;
        this.#reader = reader;
    }
    public async findFiles(start: AuthorizedPath, pattern: string, options: {
        maxResults: number;
        after?: string;
        signal?: AbortSignal;
    }): Promise<{
    ...
    public async searchText(start: AuthorizedPath, query: string, options: {
        glob?: string;
        ...
        for (const discovered of files) {
            ...
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
                ...
                while (from <= haystack.length) {
                    ...
                    const shorten = (value: string): string => value.length <= 2000 ? value : `${value.slice(0, 2000)}…[truncated]`;
